...
Airnet.prototype.generateKeyAndID = function() {
  ...
  _this.log(" - generated new identity:");
  _this.log("   -> ID = "+this.id);
};
...
Airnet.prototype.saveData = function() {
  if(this.config.usefile) {
    ...
    _this.log(" - wrote save file");
  }
  else {
    _this.log(" - no save option enabled, not saving!");
  }
}
...
Airnet.prototype.loadSettings = function(fpath) {
  ...
  try {
    ...
    _this.log(" - Loaded keys, id: "+this.id);
  } catch(err) {
    _this.log(" ! err loading keys, backing up save "+err);
    var path = this.config.file;
    fs.createReadStream(path).pipe(fs.createWriteStream(path+".bak"));
    _this.log(" - backed up, using new keys.");
...
Airnet.prototype.log = _this.log;
