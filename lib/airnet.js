...
Airnet.prototype.autoConnectThread = function(){
  //All of the peers we should attempt to connect to
  this.peersToTry = [];
  while(!this.stop){
    if(this.peersToTry.length != 0){
      var ip = this.peersToTry.pop();
      //Check if we're already connected
      if(_.findWhere(this.peers, {ip: ip}) < 0){
        this.connect(ip);
      }
    }
    sleep(100);
...
Airnet.prototype.start = function() {
  ...
  if(this.config.autoconnect){
    Fiber(this.autoConnectThread).run();
  } 
